...
class processMobile:
    ...
    def _isMobile(self,
                  phone_no):  # verify if phone_no is in correct format e.g. 703xxxxxxx else report as bad phone no.
        ...
        try:
            # print phone_no
            return result['national_format'].count(' ') == 1, result[
                'phone_number']  # return national-format and phone number
        except:
    ...
    def _addCountryCode(self, phone_no):  # adds or correcting country code like +91
        phone_no = phone_no.replace(' ', '').replace('(', '').replace(')',
                                                                      '')  # removes all white space,(,) from phone number
        ...
        for row in rows:
            ...
            for i in range(1, 6):
                ...
                if row['Phone' + str(i)] is not None:  # getting Phone1 column data from the row
                    ...
                    if len(row['Phone' + str(i)]) < 9:
                        city = row["City"]
                        processor = mobileSTDParser.processMobileSTDcode();
                        row['Phone' + str(i)] = processor.processSTD(city, row['Phone' + str(i)])

                    y = (
                    filter(lambda x: not x.isalpha(), safe_dec_enc(row['Phone' + str(i)])).strip().replace(' ', ''))
            ...
            for i in range(len(phn_no_ls)):
                if len(phn_no_ls[i]) >= 20:
                    ...
                    splt_no = phn_no_ls[i][0:-len(phn_no_ls[i]) / 2 + n] + '/' + phn_no_ls[i][
                                                                                 -len(phn_no_ls[i]) / 2 + n:]
                    phn_no_ls[i] = splt_no
                ...
                for i in range(1, 6):
                    ...
                    if isMob:
                        cleaned_no = cleaned_no.replace('+91',
                                                        '')  # removing the country code to get the actual phone number
